...
import { getTreatmentPlans, deleteTreatmentPlanById } from 'apiSdk/treatment-plans';
import { TreatmentPlanInterface } from 'interfaces/treatment-plan';

type ColumnType = ColumnDef<TreatmentPlanInterface, unknown>;

interface TreatmentPlanListPageProps {
...
export function TreatmentPlanListPage(props: TreatmentPlanListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TreatmentPlanInterface>>(
    () => `/treatment-plans?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteTreatmentPlanById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: TreatmentPlanInterface) => {
    if (hasAccess('treatment_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/treatment-plans/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/treatment-plans/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('treatment_plan', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/treatment-plans/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('treatment_plan', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="treatment_plan" />
        </Flex>

        {hasAccess('treatment_plan', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/treatment-plans/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
